...
from DataFlow::PathNode source, DataFlow::PathNode sink, InjectFilePathConfig conf, Class c, Method m
where conf.hasFlowPath(source, sink)
and
m = c.getAMethod() 
and
getControllerFunc(source.getNode().getEnclosingCallable()) = m
select c, m, source, sink, "FilePathInjection.ql", source.toString(),source.getNode().getEnclosingCallable(),source.getNode().getEnclosingCallable().getFile().getAbsolutePath(), 
